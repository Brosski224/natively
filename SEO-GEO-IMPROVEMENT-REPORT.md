...
- **SoftwareApplication JSON-LD is declared but not emitted per-page** by `buildSchemaTags` (pre-existing behavior; only Breadcrumb/Article/FAQ are injected). If per-page SoftwareApplication is desired, extend `buildSchemaTags` once and all declaring pages benefit.
