...
- **Other base-template JSON-LD still leaks onto interior pages.** The base `index.html` ships 7 static (non-Helmet) JSON-LD blocks; all are captured onto every prerendered interior page. This round fixed the SoftwareApplication leak specifically (it conflicted with the new per-page block). The homepage's `WebPage`, `VideoObject`, and homepage `FAQPage` blocks still appear on interior pages where they don't strictly belong — low-severity (crawlers tolerate it) but worth a follow-up: either move those into Helmet-managed per-page schema or extend the strip logic the same way.
